...
def validate_relative_images(paths: list[Path]) -> None:
    ...
    if missing:
        joined = "\n".join(missing)
        raise FileNotFoundError(f"unresolved relative image resources:\n{joined}")
...
def build_docx(
    ...
    *,
    ...
    if output.exists() and not force:
        raise FileExistsError(f"output already exists; use --force only after confirming overwrite: {output}")
    output.parent.mkdir(parents=True, exist_ok=True)
    TEMP_ROOT.mkdir(parents=True, exist_ok=True)
    validate_relative_images(sources)
    ...
    with tempfile.TemporaryDirectory(prefix="export-", dir=TEMP_ROOT) as raw_tmp:
        ...
        resource_dirs = list(dict.fromkeys(str(source.parent) for source in sources))
        if resource_dirs:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--force", action="store_true", help="Overwrite an existing output only after explicit confirmation.")
