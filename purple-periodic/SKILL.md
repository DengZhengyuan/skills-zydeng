...
- `待跟进事项 / 下周重点方向 / 下月重点方向` 必须谨慎收紧：只把明确承诺、明确未闭合事项、真实阻塞或高置信度延续线程写成待跟进。
- 不把普通讨论、老师追问、AI 推断出的合理改进自动改写成下周 / 下月待办；推测性内容只能写成“可关注 / 待确认 / 候选方向”，并说明证据较弱。
...
- `本周阻塞 / 待跟进事项`：由 `今日杂项 / 触发事项` 和 project log 中明确尚未闭合的状态综合判断；不把所有未解决讨论都转成待办。
- `下周重点方向`：从本周的明确阻塞、未闭合线程、项目动量和 `未来几天需要做的事情` 中提炼；推测性方向必须谨慎写成“可关注 / 待确认”，且不原样抄写短期承诺。
...
- `本月反复出现的问题 / 阻塞`：从 weekly 中重复出现的明确阻塞和 recurring friction 推断；不要把一次性提醒扩写成长期阻塞。
- `下月重点方向`：从前文的明确阻塞、项目动量、学生推进和阶段判断中自然推导；推测性方向只能写成候选或可关注，不写成已经承诺的待办。
